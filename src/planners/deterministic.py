...
UNSUPPORTED_ACTION_PATTERNS = (
    ...
    ("send customer message", r"\b(?:send|dispatch|deliver)\s+(?:(?:(?:the\s+)?customer|them)\s+)?(?:an?\s+|the\s+)?(?:customer\s+)?(?:confirmation(?:\s+message)?|message|notification|reply|response)\b"),
    ("execute payment", r"\b(?:execute|process|issue|transfer)\s+(?:a\s+|the\s+)?payment\b"),
    ("execute refund", r"\b(?:execute|process|issue|transfer)\s+(?:a\s+|the\s+)?(?:(?:(?:usd|sar|eur|gbp)\s*)?[$ۣ]?\s*\d[\d,]*(?:\.\d{1,2})?(?:\s*(?:usd|sar|eur|gbp))?\s+)?(?:full\s+|partial\s+)?refund\b"),
